...
export default function CreateEventForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. Create the event
      const res = await fetch("http://localhost:5001/api/event/create", {
        method: "POST",
      ...
      if (bannerFile) {
        ...
        await fetch(`http://localhost:5001/api/event/upload/${eid}`, {
          method: "POST",
          body: formDataObj,
        });
      }
      ...
      for (const item of agendaItems) {
        if (item.time && item.topic) {
          await fetch(`http://localhost:5001/api/event/agenda/${eid}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
    ...
    <Paper elevation={3} sx={{ p: 3, maxWidth: "900px", margin: "30px auto" }}>
      ...
      >
        ...
        <Box sx={{ width: "100%" }}>
          ...
          {agendaItems.map((item, idx) => (
            ...
            >
              <TextField
                ...
                value={item.time}
                onChange={(e) =>
                  handleAgendaChange(idx, "time", e.target.value)
                }
              />
              <TextField
                label="Topic"
                value={item.topic}
                onChange={(e) =>
                  handleAgendaChange(idx, "topic", e.target.value)
                }
              />
          ...
          <Button onClick={addAgendaItem} startIcon={<AddCircleIcon />}>
            Add Agenda Item
          </Button>
        </Box>

  ...
  const handleViewDetails = async (event) => {
    try {
      ...
      const agendaRes = await fetch(
        `http://localhost:5001/api/event/agenda/${event.eid}`
      );
      ...
      const detailRes = await fetch(
        `http://localhost:5001/api/event/details/${event.eid}`
      );
      ...
      <Dialog open={openModal} onClose={handleClose} maxWidth="md" fullWidth>
        ...
        <DialogContent dividers>
          {selectedEvent?.banner_file && (
            <img
              src={`http://localhost:5001/uploads/${selectedEvent.banner_file}`}
              alt="Event Banner"
              style={{
                width: "100%",
                height: "200px",
          ...
          <Typography variant="subtitle1">
            <strong>Category:</strong> {selectedEvent?.event_category}
          </Typography>
          <Typography variant="subtitle1">
          ...
          <Typography variant="subtitle1" sx={{ mt: 2 }}>
            ...
            {selectedEvent?.sponsor_infos || "N/A"}
          </Typography>
          <Typography variant="subtitle1">
            <strong>Speaker Info:</strong>{" "}
            {selectedEvent?.speaker_infos || "N/A"}
          </Typography>

          {/* Agenda */}
          {selectedEvent?.agenda && selectedEvent.agenda.length > 0 && (
            <>
              <Typography variant="h6" sx={{ mt: 3, color: "#235784" }}>
                Agenda
              </Typography>
              {selectedEvent.agenda.map((item, idx) => (
                <Typography key={idx} variant="body2">
                  🕒 {item.time_slot} — {item.topic}
                </Typography>
              ))}
            </>
          )}
        ...
        <DialogActions>
          <Button onClick={handleClose} sx={{ color: "#235784" }}>
            Close
          </Button>
        </DialogActions>
      </Dialog>
